...
export default function TestimonialSec({  }) {
  ...
  return (
    ...
    <section className="mt-5 mx-5 w-auto px-5 py-3 bg-black bg-opacity-40 max-w-5xl rounded-lg place-self-center border border-orange-400">
      
      <h2 className="text-3xl mt-3 text-red-600 font-bold text-center lg:text-4xl lg:text-left">Here's what others said!</h2>

      {data.allFile.edges.map(({ node }) => (
        <div
          className="flex flex-col lg:flex-row place-items-center my-5 py-5 lg:px-5 border border-gray-700"
          key={node.childMdx.id}
        >
          <div className="flex flex-col place-items-center">
            <GatsbyImage
              className="w-32 h-32 rounded-full  shrink-0 shadow-lg"
